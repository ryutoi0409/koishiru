      ...
      <div style={{ maxWidth: "1240px", margin: "0 auto" }}>
        ...
        <div className="main-grid" style={{ display: "flex", flexDirection: "column", gap: "22px" }}>
          ...
          <section style={{ position: "relative", overflow: "hidden", borderRadius: "32px", padding: "clamp(24px, 6vw, 48px)", background: "linear-gradient(180deg, rgba(255,255,255,0.05) 0%, rgba(255,255,255,0.02) 100%)", border: "1px solid rgba(255,255,255,0.08)", boxShadow: "0 24px 90px rgba(0,0,0,0.45)", flex: 1 }}>
            ...
            <div style={{ color: "#c8c8c8", fontSize: "16px", lineHeight: "1.95", maxWidth: "640px", marginBottom: "34px" }}>
              <p style={{ margin: "0 0 20px 0" }}>
                友達は関係性を壊さないために、つい優しい言葉を選んでしまうもの。コイシルは、あえて利害関係のない第3者から、残酷なほど客観的な**「恋のセカンドオピニオン」**をもらうことで、本当の状況を「知る」ための場所です。
              </p>
              <div style={{ display: "grid", gap: "10px", padding: "20px", background: "rgba(255,255,255,0.03)", borderRadius: "20px" }}>
                <div style={{ fontSize: "14px" }}><strong style={{ color: "#fff" }}>恋を知る：</strong> 自分の主観や友達の優しさというフィルターを外し、客観的な事実としての「恋の現在地」を知る。</div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px", marginBottom: "28px" }}>
              {[["01", "小さな『モヤッ』を可視化", "些細な違和感を言語化。"], ["02", "荒れない管理", "独自の質を維持。"], ["03", "恋のセカンドオピニオン", "第3者がフラットに診察。"], ["04", "恋の失敗を、みんなの教訓に", "誰かの恋の羅針盤に。"]].map(([num, title, desc]) => (
          ...
          <section className="side-section" style={{ borderRadius: "32px", padding: "clamp(20px, 5vw, 32px)", background: "linear-gradient(180deg, rgba(255,255,255,0.05) 0%, rgba(255,255,255,0.025) 100%)", border: "1px solid rgba(255,255,255,0.08)", boxShadow: "0 24px 90px rgba(0,0,0,0.45)" }}>
            <h2 style={{ margin: "0 0 24px 0", fontSize: "26px" }}>問診票を記入する</h2>
            <Field label="ペンネーム" fullWidth>
              <input placeholder="匿名可" value={postName} onChange={(e) => setPostName(e.target.value)} style={inputStyle} />
            </Field>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px", marginTop: "14px", marginBottom: "14px" }}>
              <Field label="出会い"><select style={selectStyle} value={meet} onChange={(e) => setMeet(e.target.value)}><option value="">選択</option><option>アプリ</option><option>職場</option><option>紹介</option><option>SNS</option><option>その他</option></select></Field>
              <Field label="今の関係"><select style={selectStyle} value={relationship} onChange={(e) => setRelationship(e.target.value)}><option value="">選択</option><option>片思い</option><option>友達以上</option><option>恋人</option><option>複雑な関係</option><option>元恋人</option></select></Field>
              <Field label="連絡頻度"><select style={selectStyle} value={time} onChange={(e) => setTime(e.target.value)}><option value="">選択</option><option>毎日</option><option>数日おき</option><option>たまに</option><option>相手のみ</option><option>自分のみ</option></select></Field>
              <Field label="直近の接触"><select style={selectStyle} value={length} onChange={(e) => setLength(e.target.value)}><option value="">選択</option><option>会った</option><option>電話した</option><option>LINEした</option><option>接触なし</option></select></Field>
              <Field label="診察内容"><select style={selectStyle} value={dateType} onChange={(e) => setDateType(e.target.value)}><option value="">選択</option><option>脈の有無</option><option>接し方</option><option>自分の失敗</option><option>冷められた?</option></select></Field>
              <Field label="相手の反応"><select style={selectStyle} value={reaction} onChange={(e) => setReaction(e.target.value)}><option value="">選択</option><option>積極的</option><option>受動的</option><option>普通</option><option>既読スルー</option></select></Field>
              <Field label="手応え"><select style={selectStyle} value={selfFeeling} onChange={(e) => setSelfFeeling(e.target.value)}><option value="">選択</option><option>最高</option><option>普通</option><option>脈なし</option></select></Field>
            </div>
            ...
            <Field label="タグ（最大5つ）">
              <div style={{ display: "flex", flexWrap: "wrap", gap: "6px", marginTop: "6px" }}>
                {TAG_OPTIONS.map(tag => (
                  <button key={tag} onClick={() => toggleTag(tag)} style={{ padding: "6px 12px", borderRadius: "10px", fontSize: "11px", border: "1px solid rgba(255,255,255,0.1)", background: selectedTags.includes(tag) ? "#fff" : "rgba(255,255,255,0.03)", color: selectedTags.includes(tag) ? "#000" : "#ccc", cursor: "pointer" }}>{tag}</button>
                ))}
              </div>
            </Field>

            <Field label="違和感・相談内容" fullWidth>
              <textarea placeholder="気になった言動を詳しく..." value={detail} onChange={(e) => setDetail(e.target.value)} style={textareaStyle} />
            </Field>
        ...
        <section style={{ marginTop: "48px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: "24px", gap: "10px", flexWrap: "wrap" }}>
            <h2 style={{ margin: 0, fontSize: "28px" }}>診察待ち一覧</h2>
            <div style={{ display: "flex", gap: "12px", flex: 1, justifyContent: "flex-end", alignItems: "center" }}>
              <input placeholder="検索..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} style={{ ...answerInputStyle, maxWidth: "250px", background: "rgba(255,255,255,0.05)" }} />
          ...
          <div style={{ display: "grid", gap: "32px" }}>
            {currentPosts.map((post, idx) => {
              ...
              return (
                <div key={post.id} style={{ ...postCardStyle, background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.1)", padding: "clamp(20px, 4vw, 32px)" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "16px" }}>
                    ...
                    <div style={{ display: "flex", gap: "8px" }}>
                      {post.isSolved && <span style={{ background: "#2ecc71", color: "#fff", fontSize: "10px", padding: "3px 10px", borderRadius: "6px", fontWeight: "bold" }}>解決 ✔</span>}
                      <button onClick={() => handleToggleSolved(post)} style={{ background: "none", border: "1px solid #444", color: "#888", fontSize: "10px", padding: "3px 8px", borderRadius: "6px" }}>鍵操作</button>
                    </div>
                  </div>
                  ...
                  <div className="compact-meta-container">
                    ...
                    <Meta label="関係" value={post.relationship} /><Meta label="頻度" value={post.time} /><Meta label="接触" value={post.length} /><Meta label="手応え" value={post.selfFeeling} />
                  </div>
      ...
      {isOpen && (
        <div style={{ marginTop: "12px", display: "grid", gap: "10px" }}>
          {post.answers.map((ans:any) => (
            <div key={ans.id} style={{ padding: "16px", borderRadius: "16px", background: ans.isAuthor ? "rgba(52,152,219,0.1)" : (ans.isBest ? "rgba(255,215,0,0.05)" : "rgba(0,0,0,0.3)"), border: ans.isAuthor ? "1px solid rgba(52,152,219,0.3)" : (ans.isBest ? "1px solid #ffd700" : "1px solid rgba(255,255,255,0.05)") }}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "8px" }}>
                <span style={{ fontSize: "11px", color: ans.isAuthor ? "#3498db" : "#777", fontWeight: ans.isAuthor ? "bold" : "normal" }}>{ans.isAuthor && "👤 投稿者 "}{ans.name} ({ans.attr}) · {ans.createdAt}</span>
...
function AnswerBox({ post, onAnswer, onAddUpdate }: any) {
  ...
  const handleSubmit = () => {
    if (isAuthorMode) {
      const input = prompt("秘密の鍵（4桁）を入力");
      if (input !== post.postKey) { alert("鍵が違います"); return; }
      if (confirm("これは回答への『本人返答』ですか？（はい=青色の投稿、いいえ=追記として投稿）")) {
        onAnswer(post.id, post.name, "投稿者本人", text, true);
      } else {
        onAddUpdate(post, text);
      }
    } else {
      onAnswer(post.id, name, isPrivate ? "非公開" : `${marriage}・${age}・${gender}`, text, false);
    }
    setText(""); setName("");
  };
  ...
  return (
    <div style={{ background: "rgba(0,0,0,0.2)", padding: "18px", borderRadius: "20px", border: "1px solid rgba(255,255,255,0.03)" }}>
      <div style={{ display: "flex", gap: "10px", marginBottom: "12px" }}>
        <button onClick={() => setIsAuthorMode(false)} style={{ flex: 1, padding: "10px", borderRadius: "10px", fontSize: "11px", background: !isAuthorMode ? "rgba(255,255,255,0.1)" : "transparent", border: "1px solid rgba(255,255,255,0.1)", color: "#fff" }}>一般回答者</button>
        <button onClick={() => setIsAuthorMode(true)} style={{ flex: 1, padding: "10px", borderRadius: "10px", fontSize: "11px", background: isAuthorMode ? "#3498db" : "transparent", border: "1px solid rgba(255,255,255,0.1)", color: "#fff" }}>本人として返信</button>
      </div>
      {!isAuthorMode && (
        <div style={{ marginBottom: "12px" }}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: "8px", alignItems: "center" }}>
            ...
            <select value={age} onChange={e => setAge(e.target.value)} style={smallSelectStyle} disabled={isPrivate}><option>10代</option><option>20代</option><option>30代</option><option>40代</option></select>
            ...
            <label style={{ fontSize: "11px", color: "#888", display: "flex", alignItems: "center", gap: "4px" }}><input type="checkbox" checked={isPrivate} onChange={e => setIsPrivate(e.target.checked)} /> 属性非公開</label>
          </div>
        </div>
      )}
      <textarea placeholder="答え合わせを投稿..." value={text} onChange={(e) => setText(e.target.value)} style={{...answerInputStyle, minHeight: "80px", padding: "12px"}} />
...
const Field = ({ label, children, fullWidth = false }: any) => <div style={{ marginBottom: "14px", gridColumn: fullWidth ? "1/-1" : "span 1" }}><label style={{ display: "block", fontSize: "12px", color: "#666", marginBottom: "6px", fontWeight: "bold" }}>{label}</label>{children}</div>;
const Meta = ({ label, value }: any) => value ? <span style={{ fontSize: "10px", color: "#eee", background: "rgba(255,255,255,0.08)", padding: "4px 10px", borderRadius: "8px", border: "1px solid rgba(255,255,255,0.05)" }}>{label}:{value}</span> : null;
...
const textareaStyle = { ...inputStyle, resize: "vertical" as any };
...
const postDetailStyle = { fontSize: "16px", color: "#fff", lineHeight: "1.8", whiteSpace: "pre-wrap" as any, margin: "20px 0" };
