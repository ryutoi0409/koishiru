...
export default function Home() {
  ...
  const checkAdmin = (val: string) => { setInputPass(val); setIsAdmin(val === ADMIN_PASSWORD); };
    ...
    <main style={{ minHeight: "100vh", background: "radial-gradient(circle at top, #1c1c1c 0%, #0b0b0b 42%, #050505 100%)", color: "#ffffff", padding: "clamp(20px, 5vw, 48px) clamp(12px, 3vw, 20px)", fontFamily: 'Arial, sans-serif' }}>
      <div style={{ maxWidth: "1240px", margin: "0 auto" }}>
        ...
        <div className="main-grid" style={{ display: "flex", flexDirection: "column", gap: "22px" }}>
          ...
          {/* Concept Card */}
          <section style={{ flex: 1, borderRadius: "32px", padding: "clamp(24px, 6vw, 48px)", background: "linear-gradient(180deg, rgba(255,255,255,0.05) 0%, rgba(255,255,255,0.02) 100%)", border: "1px solid rgba(255,255,255,0.08)", boxShadow: "0 24px 90px rgba(0,0,0,0.45)", position: "relative" }}>
            ...
            <p style={{ margin: "0 0 34px 0", color: "#c8c8c8", fontSize: "16px", lineHeight: "1.95" }}>友達は関係性を壊さないために、つい優しい言葉を選んでしまうもの 。コイシルは、あえて利害関係のない第3者から、残酷なほど客観的な**「恋のセカンドオピニオン」**をもらうことで、本当の状況を「知る」ための場所です。</p>
            
            <div style={{ marginBottom: "34px" }}>
              <button onClick={() => setIsConceptOpen(!isConceptOpen)} style={{ width: "100%", textAlign: "left", background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: "16px", padding: "12px 20px", color: "#fff", cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <span style={{ fontSize: "14px", fontWeight: "bold" }}>「コイシル」に込めた意味を見る</span>
                <span>{isConceptOpen ? "▲" : "▼"}</span>
              </button>
              {isConceptOpen && (
                <div style={{ marginTop: "10px", padding: "20px", background: "rgba(255,255,255,0.03)", borderRadius: "20px", border: "1px solid rgba(255,255,255,0.05)" }}>
                  <div style={{ fontSize: "14px", marginBottom: "12px" }}><strong style={{ color: "#fff" }}>恋を知る：</strong> 自分の主観や友達の優しさというフィルターを外し、客観的な事実としての「恋の現在地」を知る。</div>
          ...
          <section className="side-section" style={{ borderRadius: "32px", padding: "clamp(20px, 5vw, 32px)", background: "linear-gradient(180deg, rgba(255,255,255,0.05) 0%, rgba(255,255,255,0.025) 100%)", border: "1px solid rgba(255,255,255,0.08)", boxShadow: "0 24px 90px rgba(0,0,0,0.45)" }}>
            <h2 style={{ fontSize: "26px", marginBottom: "20px" }}>相談を投稿する</h2>
            <Field label="ペンネーム" fullWidth><input placeholder="匿名可" value={postName} onChange={(e) => setPostName(e.target.value)} style={inputStyle} /></Field>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px", margin: "14px 0" }}>
              <Field label="出会い方"><select style={selectStyle} value={meet} onChange={e => setMeet(e.target.value)}><option value="">選択</option><option>アプリ</option><option>紹介</option><option>職場</option><option>SNS</option></select></Field>
              <Field label="関係"><select style={selectStyle} value={relationship} onChange={e => setRelationship(e.target.value)}><option value="">選択</option><option>初対面</option><option>友達</option><option>恋人</option><option>セフレ</option></select></Field>
            </div>
            <Field label="秘密の鍵（4桁）"><input type="password" maxLength={4} value={postKey} onChange={e => setPostKey(e.target.value.replace(/\D/g,''))} style={{...inputStyle, border: "1px solid rgba(52,152,219,0.3)"}} placeholder="例: 1234" /></Field>
            <Field label="タグ">
              <div style={{ display: "flex", flexWrap: "wrap", gap: "6px" }}>
                {TAG_OPTIONS.map(tag => (
                  <button key={tag} onClick={() => toggleTag(tag)} style={{ padding: "6px 12px", borderRadius: "10px", fontSize: "11px", border: "1px solid rgba(255,255,255,0.1)", background: selectedTags.includes(tag) ? "#fff" : "rgba(255,255,255,0.03)", color: selectedTags.includes(tag) ? "#000" : "#ccc" }}>{tag}</button>
                ))}
              </div>
            </Field>
            <Field label="相談内容" fullWidth><textarea placeholder="気になったポイントを詳しく..." value={detail} onChange={e => setDetail(e.target.value)} style={textareaStyle} /></Field>
        ...
        <section style={{ marginTop: "48px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: "24px", gap: "10px", flexWrap: "wrap" }}>
            <h2 style={{ fontSize: "28px", margin: 0 }}>タイムライン</h2>
            <div style={{ display: "flex", background: "rgba(255,255,255,0.05)", padding: "4px", borderRadius: "12px" }}>
              ...
              <button onClick={() => setFilterTab("盛り上がり")} style={{ ...tabBtnStyle, background: filterTab === "盛り上がり" ? "rgba(255,255,255,0.1)" : "transparent" }}>HOT</button>
            </div>
          </div>
          ...
          <div style={{ display: "grid", gap: "32px" }}>
            {currentPosts.map((post) => {
              ...
              return (
                <div key={post.id} style={{ ...postCardStyle, background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.1)", padding: "clamp(20px, 4vw, 32px)", borderRadius: "32px" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "16px" }}>
                    <div style={{ color: "#cfcfcf", fontSize: "13px", fontWeight: "bold" }}>{post.name} · {post.createdAt}</div>
                    <div style={{ display: "flex", gap: "8px" }}>
                      {post.isSolved && <span style={{ background: "#2ecc71", color: "#fff", fontSize: "10px", padding: "3px 10px", borderRadius: "6px" }}>解決済み</span>}
                      <button onClick={() => handleToggleSolved(post)} style={{ background: "none", border: "1px solid #444", color: "#888", fontSize: "10px", padding: "3px 8px", borderRadius: "6px" }}>操作</button>
                    </div>
                  </div>
                  ...
                  <div style={voteContainerStyle}>
                    <div style={{ display: "flex", justifyContent: "space-between", fontSize: "11px", marginBottom: "8px" }}><span style={{ color: "#ff4d94", fontWeight: "bold" }}>脈あり {per}%</span><span style={{ color: "#888" }}>脈なし {100-per}%</span></div>
                    <div style={gaugeBarStyle}><div style={{ ...ariGaugeStyle, width: `${per}%` }} /></div>
                  ...
                  <div style={{ fontSize: "16px", color: "#fff", lineHeight: "1.8", whiteSpace: "pre-wrap" }}>{post.detail}</div>
                  ...
                  <div style={{ borderTop: "1px solid rgba(255,255,255,0.08)", paddingTop: "24px", marginTop: "24px" }}>
                    <AnswerList post={post} isAdmin={isAdmin} fetchPosts={fetchPosts} />
                    <AnswerBox post={post} fetchPosts={fetchPosts} />
                  </div>
                  
                  {/* ★【復元】アクションエリア: いいね・共有・エモリアクション */}
...
function AnswerList({ post, isAdmin, fetchPosts }: any) {
  ...
  return (
    <div style={{ marginBottom: "15px" }}>
      <button onClick={() => setIsOpen(!isOpen)} style={{ background: "rgba(255,255,255,0.08)", border: "none", color: "#fff", fontSize: "12px", padding: "12px", borderRadius: "12px", width: "100%", textAlign: "left", fontWeight: "bold" }}>
        <span>{isOpen ? "▲ 回答を閉じる" : `▼ フラットな視点を見る (${count}件)`}</span>
      </button>
      {isOpen && (
        <div style={{ marginTop: "12px", display: "grid", gap: "10px" }}>
          {post.answers.map((ans:any) => (
            <div key={ans.id} style={{ padding: "16px", borderRadius: "16px", background: ans.isAuthor ? "rgba(52,152,219,0.1)" : "rgba(255,255,255,0.02)", border: ans.isAuthor ? "1px solid rgba(52,152,219,0.3)" : "1px solid #222" }}>
              <div style={{ fontSize: "11px", color: ans.isAuthor ? "#3498db" : "#777", marginBottom: "6px" }}>{ans.isAuthor && "👤 投稿者本人 "}{ans.name} ({ans.attr}) · {ans.createdAt}</div>
...
function AnswerBox({ post, fetchPosts }: any) {
  const [name, setName] = useState(""); const [text, setText] = useState(""); const [isAuthor, setIsAuthor] = useState(false);
  const [marriage, setMarriage] = useState("未婚"); const [age, setAge] = useState("20代"); const [gender, setGender] = useState("女性");
  const [isPrivate, setIsPrivate] = useState(false);
  const submit = async () => {
    if (!text.trim()) return;
    let finalName = name || "名無し"; let attr = isPrivate ? "非公開" : `${marriage}・${age}・${gender}`;
    if (isAuthor) {
      const pass = prompt("秘密の鍵（4桁）を入力"); if (pass !== post.postKey) { alert("鍵が違います"); return; }
      finalName = post.name; attr = "投稿者本人";
    }
    await supabase.from("answers").insert([{ id: Date.now(), post_id: post.id, name: finalName, attr, text, isAuthor, createdAt: new Date().toLocaleString("ja-JP") }]);
    setText(""); setName(""); fetchPosts();
  };
  return (
    <div style={{ background: "rgba(0,0,0,0.2)", padding: "18px", borderRadius: "20px", border: "1px solid rgba(255,255,255,0.03)" }}>
      <div style={{ display: "flex", gap: "10px", marginBottom: "12px" }}>
        <button onClick={() => setIsAuthor(false)} style={{ flex: 1, padding: "10px", borderRadius: "10px", fontSize: "11px", background: !isAuthor ? "#444" : "#222", color: "#fff", cursor: "pointer" }}>フラットな視点</button>
        <button onClick={() => setIsAuthor(true)} style={{ flex: 1, padding: "10px", borderRadius: "10px", fontSize: "11px", background: isAuthor ? "#3498db" : "#222", color: "#fff", cursor: "pointer" }}>本人返信</button>
      </div>
      {!isAuthor && (
        <div style={{ marginBottom: "12px" }}>
          <input placeholder="名前" value={name} onChange={e => setName(e.target.value)} style={{...answerInputStyle, marginBottom: "8px"}} />
          <div style={{ display: "flex", flexWrap: "wrap", gap: "8px" }}>
            <select value={marriage} onChange={e => setMarriage(e.target.value)} style={smallSelectStyle} disabled={isPrivate}><option>未婚</option><option>既婚</option></select>
            <select value={age} onChange={e => setAge(e.target.value)} style={smallSelectStyle} disabled={isPrivate}><option>10代</option><option>20代</option><option>30代</option></select>
            <label style={{ fontSize: "11px", color: "#888", display: "flex", alignItems: "center", gap: "4px", cursor: "pointer" }}><input type="checkbox" checked={isPrivate} onChange={e => setIsPrivate(e.target.checked)} /> 匿名</label>
          </div>
        </div>
      )}
      <textarea placeholder="セカンドオピニオンを投稿..." value={text} onChange={(e) => setText(e.target.value)} style={{...answerInputStyle, minHeight: "80px"}} />
      <button onClick={submit} style={{...mainButtonStyle, padding: "12px", marginTop: "12px", background: isAuthor ? "#3498db" : "#fff", color: isAuthor ? "#fff" : "#000"}}>送信</button>
    </div>
  );
}
...
function Meta({ label, value }: any) { return value ? <span style={{ fontSize: "10px", color: "#eee", background: "rgba(255,255,255,0.08)", padding: "4px 8px", borderRadius: "8px", marginRight: "6px" }}>{label}:{value}</span> : null; }
...
const smallSelectStyle = { ...inputStyle, width: "auto", padding: "4px 8px", fontSize: "11px" };
...
const emptyTextStyle = { padding: "60px 20px", color: "#444", fontSize: "14px", textAlign: "center" as any };
