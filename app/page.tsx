...
export default function Home() {
  ...
  const handleAnswer = async (postId: number, name: string, attr: string, text: string) => {
    if (!text.trim()) return;
    const newAnswer = {
      id: Date.now(), post_id: postId, name: name.trim() || "名無し",
      ...
      <div style={{ maxWidth: "1240px", margin: "0 auto" }}>
        ...
        <div className="main-grid" style={{ display: "flex", flexDirection: "column", gap: "22px" }}>
          ...
          <section className="side-section" style={{ borderRadius: "32px", padding: "clamp(20px, 5vw, 32px)", background: "linear-gradient(180deg, rgba(255,255,255,0.05) 0%, rgba(255,255,255,0.025) 100%)", border: "1px solid rgba(255,255,255,0.08)", boxShadow: "0 24px 90px rgba(0,0,0,0.45)" }}>
            ...
            {notice && <div style={{...noticeBoxStyle, color: "#2ecc71"}}>{notice}</div>}
          </section>
        </div>
        ...
        <section style={{ marginTop: "40px" }}>
          ...
          <div style={{ display: "grid", gap: "24px" }}>
            {currentPosts.length === 0 ? <div style={emptyTextStyle}>該当する投稿はありません。</div> : currentPosts.map((post, index) => {
                ...
                <div key={post.id} style={postCardStyle}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "16px" }}>
                    ...
                    <div style={{ display: "flex", gap: "8px" }}>
                      ...
                      <button onClick={() => handleToggleSolved(post.id, !!post.isSolved)} style={{ background: "none", border: "1px solid #444", color: "#888", fontSize: "10px", padding: "2px 8px", borderRadius: "6px", cursor: "pointer" }}>
                        {post.isSolved ? "未解決に戻す" : "解決済みにする"}
                      </button>
                    </div>
                  </div>
...
function AnswerList({ post, isAdmin, onSetBest, onAnswerLike, onDeleteAnswer }: any) {
  ...
  return (
    <div style={{ marginBottom: "15px" }}>
      <button onClick={() => setIsOpen(!isOpen)} style={{ background: "rgba(255,255,255,0.05)", border: "none", color: "#888", fontSize: "12px", padding: "12px", borderRadius: "12px", cursor: "pointer", width: "100%", textAlign: "left", display: "flex", justifyContent: "space-between" }}>
...
function AnswerBox({ postId, onAnswer }: { postId: number; onAnswer: any }) {
  ...
  return (
    <div style={{ marginTop: "14px", padding: "18px", background: "rgba(255,255,255,0.03)", borderRadius: "20px", border: "1px solid rgba(255,255,255,0.05)" }}>
      ...
      <textarea placeholder="アドバイスを投稿..." value={text} onChange={(e) => setText(e.target.value)} style={{...answerInputStyle, minHeight: "80px", padding: "12px"}} />
      <button onClick={() => {
        const attr = isPrivate ? "非公開" : `${marriage}・${age}・${gender}`;
        onAnswer(postId, name, attr, text);
        setText(""); setName("");
      }} style={{...mainButtonStyle, padding: "12px", fontSize: "14px", marginTop: "12px"}}>回答を投稿する</button>
    </div>
  );
}
...
function UpdateInput({ onAdd }: { onAdd: (text: string) => void }) {
  ...
  if (!show) return <button onClick={() => setShow(true)} style={{ marginTop: "14px", background: "none", border: "none", color: "#3498db", fontSize: "12px", cursor: "pointer", textDecoration: "underline" }}>+ 追いコイシルを追記する</button>;
  return (
...
function Meta({ label, value }: { label: string; value: string }) {
  ...
  return (
    <div style={{ padding: "4px 10px", borderRadius: "8px", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.05)", display: "flex", alignItems: "center", gap: "4px" }}>
...
const infoCardStyle = { padding: "16px", borderRadius: "20px", background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.05)" };
...
const postCardStyle = { borderRadius: "32px", padding: "24px", background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.05)" };
const postDetailStyle = { color: "#eee", fontSize: "15px", lineHeight: "1.8", whiteSpace: "pre-wrap" as const, margin: "20px 0" };
const answerContainerStyle = { marginTop: "24px", paddingTop: "24px", borderTop: "1px solid rgba(255,255,255,0.08)" };
const answerItemStyle = { padding: "16px", borderRadius: "20px", color: "#eee", marginBottom: "12px" };
...
const voteContainerStyle = { margin: "16px 0", padding: "16px", background: "rgba(255,255,255,0.03)", borderRadius: "20px" };
...
const updateBubbleStyle = { marginTop: "12px", padding: "14px", background: "rgba(52,152,219,0.08)", borderRadius: "18px", fontSize: "14px", border: "1px solid rgba(52,152,219,0.15)" };
